...
const DIGITAL_CATEGORY_TYPES = [
  { id: 'accounts', name: 'Аккаунты', icon: '👤' },
  { id: 'subscriptions', name: 'Подписки', icon: '⭐' },
  { id: 'services', name: 'Услуги', icon: '🛠️' },
  { id: 'game_items', name: 'Игровые предметы', icon: '🎮' },
  { id: 'other', name: 'Прочее', icon: '📦' },
];

const PHYSICAL_CATEGORY_TYPES = [
  { id: 'food', name: 'Еда', icon: '🍔' },
  { id: 'clothes', name: 'Одежда', icon: '👕' },
  { id: 'services', name: 'Услуги', icon: '🛠️' },
  { id: 'auction', name: 'Аукцион', icon: '🔨' },
  { id: 'other', name: 'Прочее', icon: '📦' },
];

interface CategoriesManagerNewProps {
  storeId: string;
  storeType: StoreType;
  onBack: () => void;
}

export const CategoriesManagerNew = ({ storeId, storeType, onBack }: CategoriesManagerNewProps) => {
  const [categories, setCategories] = useState<Category[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [showLanguageTab, setShowLanguageTab] = useState<'ru' | 'en'>('ru');
  ...
  const categoryTypes = storeType === 'digital' ? DIGITAL_CATEGORY_TYPES : PHYSICAL_CATEGORY_TYPES;
  const emojiList = ['📦', '🎮', '👕', '🍔', '📱', '💻', '🎧', '📚', '⚽', '🎨', '🏠', '🚗', '💎', '🎁', '🌟'];
  ...
  const handleSubmit = async () => {
    if (!formData.name.trim()) {
      webApp?.showAlert('Введите название категории на русском');
      return;
    }

    if (!formData.categoryType && !selectedParentId) {
      webApp?.showAlert('Выберите тип категории');
      return;
    }
    ...
    try {
      if (editingCategory) {
        const updateData: any = {
          ...
          category_type: selectedParentId ? null : formData.categoryType,
  ...
  const resetForm = () => {
    ...
    setFormData({ name: '', nameEn: '', icon: '📦', imageUrl: '', categoryType: '' });
    setUseCustomImage(false);
    setShowLanguageTab('ru');
  };
  ...
  const getCategoryTypeName = (typeId: string) => {
    return categoryTypes.find(ct => ct.id === typeId)?.name || typeId;
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-20">
      ...
      <div className="p-4">
        {parentCategories.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500 dark:text-gray-400 mb-4">Нет категорий</p>
                      ...
                      <div className="flex items-center gap-3 flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          {category.category_type && (
                            <span className="inline-block mt-1 text-xs px-2 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded">
                              {getCategoryTypeName(category.category_type)}
                            </span>
                          )}
                          {!category.is_visible && (
                            <span className="inline-block mt-1 ml-2 text-xs text-gray-500 dark:text-gray-400">Скрыта</span>
                          )}
                        </div>
                      </div>
      ...
      <Modal isOpen={showModal} onClose={resetForm} title={editingCategory ? 'Редактировать' : (selectedParentId ? 'Добавить подкатегорию' : 'Добавить категорию')}>
        <div className="space-y-4">
          {!selectedParentId && !editingCategory?.parent_category_id && (
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {categoryTypes.map((type) => (
                  <button
                    key={type.id}
                    onClick={() => setFormData(prev => ({ ...prev, categoryType: type.id, icon: type.icon }))}
                    className={`p-3 rounded-lg border-2 transition-colors ${
                      formData.categoryType === type.id
                        ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/30'
                        : 'border-gray-200 dark:border-gray-600 hover:border-gray-300'
                    }`}
                  >
                    <div className="text-2xl mb-1">{type.icon}</div>
                    <div className="text-xs font-medium text-gray-900 dark:text-white">{type.name}</div>
                  </button>
                ))}
              </div>
            </div>
